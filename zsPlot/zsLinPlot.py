...
"""
Filename: zsLinPlot.py
Purpose: This program plots the linear radar reflectivity (mm^6/m^3) against 
...
while i < 3:
    if i == 0:
        j=0
        while j < 4:
            plt.plot(zLinD, sD[j,:],label = zsDName[j])
            j=j+1
        plt.ylabel('Estimated Snowfall (mm/hr)')
        plt.xlabel('Reflectivity ($mm^6$/$m^3$)')
        plt.xscale('log')
        plt.yscale('log')
        plt.legend(loc = 2)
        plt.savefig("figZSRelLinBook.png")
        plt.close()
    ...
    if i == 1:
        ...
        plt.xscale('log')
        plt.yscale('log')
        plt.savefig("figZSRelLinGeog.png")
        plt.close()
    ...
    if i == 2:
        ...
        plt.yscale('log')
        plt.legend(loc = 2)
        plt.savefig("figZSRelLinRad.png")
        plt.close()
    i=i+1    