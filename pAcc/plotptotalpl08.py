...
"""
Filename: plotptotal.py
Purpose: This program plots the precipitation accumulation for 16 Z-S 
         relationships as well as the pluvioaccumulation
         for all events at a single station.
...
#Splits z-s rel input
i=0
while i < nzs:
    zs = zsF[i].split()
    rel.append(zs[0]) #z-s rel name
    i=i+1

#Loops over each event to parse data and create graphs
ev = 0
while ev < nev:
    #Splits and initializes necessary data
    ...
    while t < nMin:
        ...
        i = 0
        #For each zs rel, prate data is saved
        while i < nzs:
            data[t,i] = prate[4+i]
            i=i+1
        j = 0
        #For each pluvio correction, data is saved
        while j < nplu:
    ...
    yAx = 60
    #Sets Y-Axis max to fit event data
    if ev == 3:
        yAx = 20
    elif ev == 8:
        yAx = 5
    ...
    while i < 4:
        if i == 0:
            ...
            plt.ylabel('Snow Acc. (mm)', fontsize=8)
            plt.ylim(0,yAx)
        ...
        if i == 2:
            ...
            plt.ylabel('Snow Acc. (mm)', fontsize=8)
            plt.xlabel('Day of Year ({0})'.format(int(dates[0])), fontsize=8)
            plt.ylim(0,yAx)
            plt.xticks(fontsize=9)
            plt.yticks(fontsize=9)
            plt.legend(loc = 2,prop={'size': 6})            
        ...
        if i == 3:
            ...
            j=12
            while j < 16:
                plt.plot(time, data[:,j],label = rel[j])
                j=j+1
        ...
        i=i+1
    plt.suptitle('Station ' + station[0].rstrip(), fontsize = 12)
    plt.savefig("fig{0}PAcc{1}.png".format(station[0].rstrip(),day))
    plt.close()
    ev = ev+1